...
const MENU: { to: string; label: string; group: string }[] = [
  {
    group: "This funnel",
    to: "/booking-flow",
    label: "Booking Flow — full screen",
  },
  {
    group: "This funnel",
    to: "/booking-flow-100x",
    label: "Booking Flow 100x",
  },
  {
    group: "This funnel",
    to: "/closing",
  ...
  },
  {
    group: "This funnel",
    to: "/final-moment",
    label: "Draft Vision (screenshots)",
  },
  {
    group: "Lead OS",
    to: "/flow-os",
    label: "Flow OS — Lead OS",
  },
  {
    group: "Lead OS",
    to: "/final-e2e-plus",
    label: "Final E2E Plus",
  },
  {
    group: "Lead OS",
    to: "/mymoves",
...
export function SplitFlow({
  ...
  useEffect(() => {
    ...
    const move = (e: PointerEvent) => {
      const pct = Math.min(
        100,
        Math.max(25, Math.round((e.clientX / window.innerWidth) * 100)),
      );
  ...
  const queue = useMemo(() => {
    ...
    return leads
      .filter(
        (l) =>
          l.stage !== "CLOSED" &&
          l.f?.["checkinDay"] !== "CHECKED_IN",
      )
      .map((l) => ({ l, h: health(l) }))
      .sort(
        (a, b) =>
          Number(b.h.sla === "LATE") - Number(a.h.sla === "LATE") ||
  ...
  useEffect(() => {
    if (lead) {
      setScreenId(currentScreen(lead.f ?? {}).id);
    }
  }, [lead?.id]);
  ...
  useEffect(() => {
    ...
    if (!match) {
      setLeadId("");
      toast.error(
        "This customer is not linked to Booking Flow yet. No duplicate was created.",
      );
      return;
    }
  ...
  const screen =
    ...
    (lead ? currentScreen(lead.f ?? {}) : SCREENS[0]!);
  ...
  const stats = useMemo(() => {
    ...
    leads.forEach((l) =>
      l.events.forEach((e) => {
        ...
        if (/call|talk|spoke|phone/i.test(`${e.label} ${e.detail ?? ""}`)) {
          calls += 1;
        }
      }),
    );

    const left = leads.filter(
      (l) =>
        l.stage !== "CLOSED" &&
        (!l.owner || !l.nextAction || !l.nextActionAt),
    ).length;
    ...
    return {
      ...
      saved,
      left,
      tower: hs.filter((x) => x.toTower).length,
      late: hs.filter((x) => x.sla === "LATE").length,
    };
  }, [leads, mounted]);
   ...
   * Smart Next Move
   *
   * Reuses the existing Booking Flow data instead of creating a
   * second persistence mechanism.
   */
  function getRecommendedAction() {
    if (!lead) return "Follow up on decision";

    if (lead.nextAction) {
      return lead.nextAction;
    }

    if (h?.waitingOn === "Customer") {
      return "Call the customer";
    }

    if (h?.waitingOn === "Gharpayy") {
      return "Send WhatsApp message";
    }

    return "Follow up on decision";
  }

  function lockSmartNextMove() {
    if (!lead) return;

    const action = getRecommendedAction();

    const at = lead.nextActionAt
      ? new Date(lead.nextActionAt).toISOString()
      : new Date(Date.now() + 2 * 3_600_000).toISOString();

    setNext(lead.id, action, at);
    ...
    // Move to the next customer after the state update has been initiated.
    window.setTimeout(() => {
      nextCustomer();
    }, 150);
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <header className="shrink-0 border-b px-2 py-1">
          <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2">
            <div className="flex min-w-0 items-center gap-1 overflow-hidden">
              <h1 className="shrink-0 text-xs font-semibold">
                Booking Flow
              </h1>
              ...
              {mounted && (
                <div className="flex min-w-0 gap-1 overflow-x-auto">
                  ...
                  >
                    <PhoneCall className="mr-0.5 h-2.5 w-2.5" />
                    {stats.calls} calls
                  </Badge>
                        ...
                        return MENU.map((m) => (
                          <div key={m.to}>
                            {m.group !== lastGroup &&
                              ((lastGroup = m.group),
                              (
                                <p className="px-2 pb-0.5 pt-1.5 text-[9px] font-semibold uppercase tracking-wide text-muted-foreground">
                                  {m.group}
                                </p>
                              ))}
        ...
        {/* Customer line + the five answers */}
        {lead && (
            ...
            {mounted && h && (
              <div className="mt-1 flex flex-wrap gap-1 text-[10px]">
                <Badge variant="outline" className="text-[10px]">
                  {h.stepNo}. {h.complete ? "Checked in" : h.step?.title}
                </Badge>

                <Badge
                  variant={lead.owner ? "secondary" : "destructive"}
                  className="text-[10px]"
                >
                  {lead.owner ?? "no owner"}
                </Badge>
                ...
                <Badge
                  variant={lead.nextAction ? "outline" : "destructive"}
                  className="text-[10px]"
                >
                  {lead.nextAction ?? "no next step"}
                </Badge>

                <Badge
                  variant={
                    lead.nextActionAt && h.sla !== "LATE"
                      ? "outline"
                      : "destructive"
                  }
                  className="text-[10px]"
          ...
          {pane === "CLOSING" ? (
            ...
            <div className="space-y-2">
              <p className="text-[10px] text-muted-foreground">
                Four drafts a day for {me} — D1, D2, D3, D4 · {BATCH_SIZE}{" "}
                customers each. Close a draft when all 30 have a next step
                and a deadline.
              </p>

              {ROUNDS.map((r) => {
                const batch = batches.find(
                  (b) => b.handler === me && b.round === r,
                );
                    ...
                    <div className="flex items-center justify-between gap-2">
                      <div className="min-w-0">
                        <p className="text-xs font-semibold">
                          D{r} · {rows.length || BATCH_SIZE} customers
                        </p>
                        ...
                        <p className="truncate text-[10px] text-muted-foreground">
                          {!batch
                            ...
                            : batch.closedAt
                              ...
                              : `${done}/${rows.length} have a next step and deadline`}
                        </p>
                      </div>
                        ...
                        {!batch ? (
                          <Button
                            ...
                            onClick={() => {
                              ...
                              toast[made ? "success" : "error"](
                                made
                                  ? `D${r} opened with ${made.leadIds.length} customers`
                                  : "No customers left to fill this draft",
                              );
                            }}
                          >
                            Open D{r}
            ...
            <div className="space-y-1.5">
              <p className="text-[10px] text-muted-foreground">
                {queue.length} customers still need a decision — worst first.
              </p>
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      ...
                      >
                        ...
                        <p className="truncate text-[10px] text-muted-foreground">
                          {lh.stepNo}.{" "}
                          {lh.step?.title ?? "Checked in"} ·{" "}
                          {l.owner ?? "no owner"} ·{" "}
                          {l.nextAction ?? "no next step"}
                        </p>
                      </button>
            ...
            <p className="pt-10 text-center text-sm text-muted-foreground">
              Nothing left in the queue — every customer is closed or checked
              in.
            </p>
          ) : pane === "WORK" ? (
            <ScreenPanel
        ...
        {/* Smart Next Move — recommended action for the current customer */}
        {lead && (
          <div className="shrink-0 border-t bg-background px-2 py-1.5">
            <div className="flex items-center gap-2 rounded-md border border-primary/20 bg-primary/5 px-2 py-1.5">
            ...
            <div className="flex items-center gap-1 overflow-x-auto">
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  toast.success("Control Tower notified");
                }}
              >
                Tower
              </Button>
              ...
              <select
                className="h-7 min-w-[8rem] flex-1 rounded-md border bg-background px-1 text-[10px]"
                value={nextAction}
                onChange={(e) =>
              ...
              <div className="space-y-3">
                ...
                <label className="block text-xs font-medium">
                  ...
                  <Input
                    className="mt-1"
                    autoFocus
                    placeholder="Outcome, promise, blocker or detail…"
                    value={activityNote}
                    onChange={(e) =>
                    ...
                    onKeyDown={(e) => {
                      if (
                        ...
                        toast.success(
                          "Activity added to the customer story",
                        );
                      }
                    }}
                  />
          ...
          <DialogContent className="max-w-sm">
            ...
            <label className="block text-xs font-medium">
              ...
              <Input
                className="mt-1"
                autoFocus
                placeholder="30 customers worked, 6 tours set…"
                value={closeNote}
                onChange={(e) =>
        ...
        <>
          ...
          <div className="flex min-w-0 flex-1 items-center justify-center bg-muted/30 p-4 text-center">
            <p className="text-[11px] text-muted-foreground">
              Keep WhatsApp Web open in this space.
              <br />
              Drag the grey bar, or use the width buttons,
              to set the sizes you want.
            </p>
          </div>
        </>
      )}
